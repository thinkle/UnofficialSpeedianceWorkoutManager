...
function normalizeExistingWorkout(workout) {
  ...
  const exercises = (workout.actionLibraryList || []).map((action) => {
    ...
    const presetId =
      rawPresetId === undefined || rawPresetId === null || rawPresetId === ""
        ? -1
...
function Builder() {
    ...
    const loadLibrary = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setStatus({
            type: "error",
            message: response.error || "Failed to load library.",
          });
        }
      } catch (error) {
        if (!isMounted) return;
  ...
  useEffect(() => {
    ...
    const loadWorkout = async () => {
      ...
      if (!response.ok) {
        setStatus({
          type: "error",
          message: response.error || "Failed to load workout.",
        });
        return;
      }
  ...
  const baseExercises = useMemo(() => {
    ...
    const matched = (library.exercises || []).filter((exercise) => {
      ...
      // Use smart equipment filter with OR/AND logic
      if (!matchesEquipmentFilter(exercise.equipment_name, equipmentFilter)) {
        return false;
      }
      if (deviceFilter !== "all") {
        const devices = String(
          exercise.device_type_tag || exercise.device_type || "",
        )
          .split(",")
          .map((value) => value.trim());
    ...
    return matched.sort(
      (a, b) => scoreRelevance(b, words) - scoreRelevance(a, words),
    );
  ...
  useEffect(() => {
    ...
    const loadDetailData = async () => {
      ...
      if (!hasAllDetails) {
        ...
        setDetailMessage(
          `Loading details for ${candidateIds.length} exercises...`,
        );
  ...
  const handleMoveCircuit = useCallback(
    (blockIndex, direction) => {
      const newExercises = moveCircuit(
        exercises,
        blockIndex,
        blockIndex + direction,
      );
  ...
  const handleRemoveRound = useCallback(
    (blockIndex) => {
      ...
      if (block && block.rounds > 1) {
        const newExercises = deleteSetFromCircuit(
          exercises,
          blockIndex,
          block.rounds - 1,
        );
  ...
  const handleRemoveCircuitExercise = useCallback(
    (blockIndex, exerciseIndex) => {
      const newExercises = deleteExerciseFromCircuit(
        exercises,
        blockIndex,
        exerciseIndex,
      );
  ...
  const handleUpdateCircuitSet = useCallback(
    (blockIndex, positionIndex, roundIndex, setIndex, updates) => {
      ...
      const exerciseIdx =
        block.start + positionIndex + roundIndex * block.cycleLen;
      setExercises((prev) => {
        const newExercises = [...prev];
  ...
  const activeExport =
    exportMode === "condensed" ? exportCondensed : exportJson;
      ...
      {showPrompt ? (
        <div className="builder-modal">
          <div className="builder-modal-card builder-modal-wide">
            ...
            <div className="builder-muted">
              Edit the instructions below freely, then click{" "}
              <strong>Append Exercises</strong> to attach the exercise list (
              {promptPool.length} exercises from current filters
              {promptPool.length > 200
                ? " — filter by category or equipment to narrow"
                : ""}
              ). Paste the result into ChatGPT or Claude, then use Import JSON
              with the response.
            </div>
            <textarea
              value={promptText}
